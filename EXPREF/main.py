...
def ocean_info():
    ...
    tunnel_config.append( { 'label' : 'TO_NEMO_FIELDS', \
                            'grids' : { 'ORCA2' : {'npts' : (180,148) } }, \
                            'exchs' : [ {'freq' : 10800, 'grd' : 'ORCA2', 'lvl' : 1, 'in' : ['sst'], 'out' : ['pysst']}, \
                                        {'freq' : 10800, 'grd' : 'ORCA2', 'lvl' : 3, 'in' : ['u'], 'out' : ['pyu']} ] }
                        )
...
def production():
    ...
    def loop_core(**inputs):
        outputs = {}
        outputs['pysst'] = add_100( inputs['sst'] )
        outputs['pyu'] = add_100( inputs['u'] )
        return outputs
