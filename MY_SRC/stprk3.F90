...
!!$      IF( ln_isf     )   CALL isf_stp    ( kstp, Nbb )                     ! update iceshelf geometry!clem: problem with isf and cpl: sbcfwb needs isf but isf needs fwf from sbccpl
         

      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
      ! Python coupling
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
      CALL send_to_python('u', uu(:,:,:,1), kstp)
      CALL send_to_python('sst', ts(:,:,1,1,1), kstp )
      CALL receive_from_python('pysst', fld_2D, kstp)
      CALL receive_from_python('pyu', fld_3D, kstp)
      CALL iom_put('pysst', fld_2D)
      CALL iom_put('pyu', fld_3D)
      !
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
